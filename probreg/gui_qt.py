...
def show_dialog(win, cls, args=None):
    ...
    # ok = False
    ok = cls(win, args).exec() if args else cls(win).exec()
...
class PageGui(qtw.QFrame):
    ...
    def move_cursor_to_end(self):
        ...
        self.text1.moveCursor(gui.QTextCursor.MoveOperation.End, gui.QTextCursor.MoveMode.MoveAnchor)
...
class Page0Gui(PageGui):
    ...
    def get_item_by_id(self, item_id):
        ...
        items = self.p0list.findItems(item_id, core.Qt.MatchFlag.MatchExactly, 0)
        if items:
            return items[0]
        return None
    ...
    def get_selected_action(self):
        ...
        return self.p0list.currentItem().data(0, core.Qt.ItemDataRole.UserRole)
...
class Page6Gui(PageGui):
    ...
    def get_textfield_contents(self):
        ...
        return self.progress_text.get_contents()
    ...
    def set_listitem_data(self, itemindex):
        ...
        self.progress_list.item(itemindex).setData(itemindex - 1, core.Qt.ItemDataRole.UserRole)
    ...
    def move_cursor_to_end(self):
        ...
        self.progress_text.moveCursor(gui.QTextCursor.MoveOperation.End,
                                      gui.QTextCursor.MoveMode.MoveAnchor)
    ...
    def convert_text(self, text, to):
        ...
        if to == 'rich':
            self.progress_text.set_contents(text)
            retval = self.progress_text.get_contents()
        else:  # if to == 'plain':
...
class SortOptionsDialog(qtw.QDialog):
    ...
    def __init__(self, parent, args):
        ...
        while row < len(lijst):
            ...
            rbg.addButton(rba, self._asc_id)
            grid.addWidget(rba, row, 2)
            rbd = qtw.QRadioButton(" Desc ", self)
            rbg.addButton(rbd, self._desc_id)
    ...
    def set_defaults(self):
        ...
        if self.parent.master.saved_sortopts:
...
class SelectOptionsDialog(qtw.QDialog):
    ...
    def __init__(self, parent, args):
        ...
        self.buttonbox = qtw.QDialogButtonBox(qtw.QDialogButtonBox.StandardButton.Ok
                                              | qtw.QDialogButtonBox.StandardButton.Cancel)
    ...
    def on_checked(self, arg):  # , val):
        ...
        else:  # if arg == 'arch':   iets anders niet mogelijk
...
class SettOptionsDialog(qtw.QDialog):
    ...
    def __init__(self, parent, args):
        ...
        title = shared.app_title
        if len(args) > 1:
            with contextlib.suppress(IndexError):  # wat er niet is is er niet
                self.cls = args[0]
                title = args[1]
                size = args[2]
        if not self.cls:
            raise ValueError('Option type not provided')
        super().__init__(parent)
...
class LoginBox(qtw.QDialog):
    ...
    def __init__(self, parent):
        ...
        self.t_password.setEchoMode(qtw.QLineEdit.EchoMode.Password)
